...
void CommProtocol::processMessage(const String &msg) {
    ...
    // === Elecrow → Stamp S3 commands (received by Stamp) ===
    // These are handled by the Stamp's comm protocol, not here.
    // But if this code runs on Stamp, handle them:
    ...
    // === Stamp S3 → Elecrow responses ===
    ...
    else if (msg.startsWith("WIFI_STATUS:connected,")) {
        String params = msg.substring(21);
        int comma = params.indexOf(',');
        if (comma > 0) {
            String ssid = params.substring(0, comma);
            String ip = params.substring(comma + 1);
            Serial.printf("[Comm] WiFi Connected: %s, IP: %s\n", ssid.c_str(), ip.c_str());
            updateWifiStatus(("Connected: " + ssid).c_str());
            ui_set_wifi(true);
        }
    }
    
    else if (msg == "WIFI_STATUS:disconnected") {
        Serial.println("[Comm] WiFi Disconnected");
        updateWifiStatus("Disconnected");
        ui_set_wifi(false);
    }
    
    else if (msg == "WIFI_STATUS:failed") {
        Serial.println("[Comm] WiFi Connection Failed");
        updateWifiStatus("Connection Failed");
    }
        ...
        if (comma1 > 0 && comma2 > 0) {
            ...
            int32_t rssi = params.substring(comma1 + 1, comma2).toInt();
            bool encrypted = params.substring(comma2 + 1) == "1";
            addNetworkToList(ssid, rssi, encrypted);
        }
    }
    
    else if (msg.startsWith("SCAN_DONE:")) {
        int count = msg.substring(10).toInt();
        Serial.printf("[Comm] Scan Done: %d networks\n", count);
        scanComplete(count);
    }
    
    else if (msg.startsWith("TIME:")) {
        String timeStr = msg.substring(5);
        Serial.printf("[Comm] Time: %s\n", timeStr.c_str());
        ui_set_time(timeStr.c_str());
    }
    
    else if (msg == "WEATHER:ERROR") {
        Serial.println("[Comm] Weather Error");
        ui_set_outdoor_aqi(0);
    }
    
    else if (msg.startsWith("WEATHER:")) {
        int aqi = msg.substring(8).toInt();
        Serial.printf("[Comm] Weather AQI: %d\n", aqi);
